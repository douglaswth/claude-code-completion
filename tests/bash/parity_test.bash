...
extract_ps_extra_flags() {
    # Scoped to the ClaudeExtraFlags block: the script has more than one
    # [pscustomobject] array, and an unscoped grep swept the bundled
    # subcommands in here too.
    sed -n '/^\$script:ClaudeExtraFlags = @(/,/^)/p' "$PS_SCRIPT" \
        | grep -E '^\s+\[pscustomobject\]@\{' | while IFS= read -r line; do
...
# Hidden subcommands are a second bundled list with the same cross-shell
# contract as the flags: both scripts must carry the same names and
# descriptions, or one shell completes a command the other does not.
extract_bash_extra_subcommands() {
    (
        source "$BASH_SCRIPT" >/dev/null 2>&1
...
extract_ps_extra_subcommands() {
    sed -n '/^\$script:ClaudeExtraSubcommands = @(/,/^)/p' "$PS_SCRIPT" \
        | grep -E '^\s+\[pscustomobject\]@\{' | while IFS= read -r line; do
        local name desc
        name=$(echo "$line" | sed -n "s/.*Name='\([^']*\)'.*/\1/p")
        desc=$(echo "$line" | sed -n "s/.*Description='\([^']*\)'.*/\1/p")
...
function test_bundled_subcommand_lists_match() {
    local bash_out ps_out
    bash_out="$(extract_bash_extra_subcommands)"
    ps_out="$(extract_ps_extra_subcommands)"
